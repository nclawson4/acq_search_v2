...
export function SearchView({ mode }: { mode: Mode }) {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100">
      {/* Persistent top-right links: embedding comparison + source code.
          Stay visible while scrolling through the long-form sections. */}
      <div className="fixed top-4 right-4 z-50 flex items-center gap-2">
        <Link
          href="/embeddings"
          className="inline-flex items-center gap-1.5 rounded-full border border-zinc-300 dark:border-zinc-700 bg-white/80 dark:bg-zinc-900/80 backdrop-blur px-3 py-1.5 text-xs font-medium text-zinc-700 dark:text-zinc-200 shadow-sm hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" className="h-4 w-4">
            <path d="M3 3v18h18" />
            <path d="M7 14l4-4 4 4 5-5" />
          </svg>
          <span>Embedding comparison</span>
        </Link>
        <a
